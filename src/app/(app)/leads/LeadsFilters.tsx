...
export function LeadsFilters({
  ...
  const sourceOptions = ALL_SOURCES.map((s) => ({ value: s, label: t.source[s] }));
  const currentSources = (current.source ?? "").split(",").filter(Boolean);
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      <MultiFilterSelect
        ...
        selectedLabel={t.leads.sourcesSelected}
        clearLabel={t.leads.clearFilter}
        values={currentSources}
        options={sourceOptions}
        disabled={isPending}
        onChange={(values) => updateParam("source", values.join(","))}
      />
      <FilterSelect
...
// Igual que FilterSelect, pero permite elegir varios valores a la vez (por
// ejemplo Apollo + Adzuna) — se abre como un desplegable con checkboxes en
// vez de un <select> nativo, que no soporta multi-selección de forma
// utilizable. Los valores elegidos viajan a `onChange` como un array; quien
// llama decide cómo guardarlos (acá, unidos por coma en la URL).
  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className={
          "flex items-center rounded-xl border py-2 pl-3 pr-9 text-sm font-medium shadow-card focus:border-dolphin-500 focus:outline-none focus:ring-1 focus:ring-dolphin-500 disabled:opacity-60 " +
          (isActive
            ? "border-dolphin-200 bg-dolphin-50 text-dolphin-700"
            : "border-slate-300 bg-white text-ink")
        }
      ...
      {open && (
        <div className="absolute left-0 z-20 mt-1.5 w-56 rounded-xl border border-slate-200 bg-white p-1.5 shadow-lg">
          {options.map((o) => (
            <label
              key={o.value}
              className="flex cursor-pointer items-center gap-2 rounded-lg px-2.5 py-1.5 text-sm text-ink hover:bg-slate-50"
            >
              <input
                type="checkbox"
          ...
          {isActive && (
            <>
              ...
              <button
                type="button"
                onClick={() => onChange([])}
                className="w-full rounded-lg px-2.5 py-1.5 text-left text-xs font-medium text-slate-500 hover:bg-slate-50 hover:text-dolphin-600"
              >
                {clearLabel}
              </button>
            </>
